...
public class TestDbUtil {
    ...
    void clearTimesheetTable() {
        this.jdbcTemplate.execute("delete from timesheet.days_worked");
    }

    int countAllInTimesheetTable() {
        return jdbcTemplate.queryForInt("select count(*) from timesheet.days_worked");
    }

    void insertDayInTimesheetTable(String date) {
        try {
            jdbcTemplate.update("insert into timesheet.days_worked values (?)", getDateTime(date));
    ...
    List<Date> getAllDatesFromTimesheetTable() {
        return jdbcTemplate.queryForList("select day from timesheet.days_worked", Date.class);
    }

    Date getSingleDateFromTimesheetTable() {
        return (Date) jdbcTemplate.queryForObject("select day from timesheet.days_worked", Date.class);
    }
